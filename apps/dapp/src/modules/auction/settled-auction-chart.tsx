...
const getCssColor = (color: string) => `hsl(${getCssVariable(color)})`;
...
export const SettledAuctionChart = ({ auction }: { auction: BatchAuction }) => {
  ...
  const primary500 = getCssColor("--primary-500");
  const red500 = getCssColor("--red-500");
  const textSecondary = getCssColor("--text-secondary");
  const textTertiary = getCssColor("--text-tertiary");
  const neutral400 = getCssColor("--neutral-400");
